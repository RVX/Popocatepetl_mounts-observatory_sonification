...
**Satellite time series** -- SO2 mass, thermal hot pixels, InSAR deformation and
coherence, all four sonified from the same real dataset (run
`python POPO01.py satellite --play` to generate and listen to the `.wav`
files locally -- they aren't committed to the repo, see
[Output folder layout](#output-folder-layout)):

![Popocatepetl MOUNTS satellite time series 2015-2026](docs/images/popo_satellite.png)

**Station / volcano context map**, generated by the `map` action:
...
3. **Plot** `do_satellite_plot()` -- one panel per series, saved to
